...
function DailyWorkoutPreview({ workout, exerciseNames, mappingRate, onAccept, onRegenerate, isAccepting }: { workout: AIDailyWorkout; exerciseNames: Map<string, string>; mappingRate: number; onAccept: () => void; onRegenerate: () => void; isAccepting: boolean }) {
  ...
  return (
    <div className="space-y-5">
      <section className="glass-card rounded-3xl border border-primary/20 bg-gradient-to-br from-primary-soft via-card to-card p-5 sm:p-6">
      ...
      <section className="space-y-2.5">
        {workout.exercises.map((exercise, index) => {
          ...
          return <div key={`${exercise.variationId}-${index}`} className="glass-card flex items-center gap-3 rounded-2xl border bg-card p-3.5 sm:p-4"><div className="grid size-9 shrink-0 place-items-center rounded-full bg-primary-soft font-mono text-xs font-semibold text-primary">{index + 1}</div><div className="min-w-0 flex-1"><p className="truncate text-sm font-semibold">{exerciseNames.get(exercise.variationId) ?? (isVi ? "Bài tập" : "Exercise")}</p><p className="mt-1 text-xs text-muted-foreground">{exercise.sets} sets × {reps} reps{exercise.rir != null ? ` · RIR ${exercise.rir}` : ""}{exercise.restTime ? ` · ${isVi ? "nghỉ" : "rest"} ${exercise.restTime}s` : ""}</p></div><Check className="size-4 shrink-0 text-success-text" /></div>
        })}
      </section>
