export const commonMessages = {
  en: {
    common: {
      language: "Language",
      theme: "Theme",
      themeLight: "Light",
      themePerformanceGreen: "Performance Green",
      themeElectricBlue: "Electric Blue",
      themeVoltLime: "Volt Lime",
      themeIronOrange: "Iron Orange",
      themeBlackVolt: "Black + Volt",
      themeCrimsonPerformance: "Crimson Performance",
      themeDark: "Dark",
      themeSystem: "System",
      english: "EN",
      vietnamese: "VI",
      openNavigation: "Open navigation",
      closeNavigation: "Close navigation",
      expandSidebar: "Expand sidebar",
      collapseSidebar: "Collapse sidebar",
      settings: "Settings",
      save: "Save",
      saving: "Saving...",
      cancel: "Cancel",
      dismissNotification: "Dismiss notification",
      edit: "Edit",
      delete: "Delete",
      loading: "Loading...",
      tryAgain: "Try again",
      today: "Today",
      selectedDay: "Selected Day",
      profile: "Profile",
      signOut: "Sign Out",
      signingOut: "Signing Out...",
      addCoach: "Add Coach",
      saveChanges: "Save Changes",
      sendResetEmail: "Send Password Reset Email",
      sendingEmail: "Sending email...",
      noData: "No data available",
    },
    shell: {
      dashboard: "Dashboard",
      weeklySchedule: "Weekly Schedule",
      workout: "Workout",
      mealTracking: "Meal Tracking",
      progress: "Progress",
      trainees: "Trainees",
      clients: "Clients",
      programs: "Programs",
      exercises: "Exercises",
      stats: "Stats",
      analytics: "Analytics",
      adminDashboard: "Admin Dashboard",
      overview: "Overview",
      users: "Users",
      coachRequests: "Coach Requests",
      coachSignups: "Coach Signups",
      connections: "Connections",
      audit: "Audit",
      controlCenter: "Control center",
      account: "Account",
      backToAthleteView: "Back to athlete view",
      addClient: "Add client",
      activeClients: "active clients",
      home: "Home",
      coach: "Coach",
      meals: "Meals",
      schedule: "Schedule",
      workouts: "Workouts",
      nutrition: "Nutrition",
      routine: "Routine",
      more: "More",
      admin: "Admin",
      routines: "Routines",
      records: "Records",
      body: "Body",
      startWorkout: "Start",
      yeahBuddyUser: "YeahBuddy User",
      loadingEmail: "Loading...",
      loadingWorkspace: "Loading workspace",
    },
  },
  vi: {
    common: {
      language: "Ngôn ngữ",
      theme: "Giao diện",
      themeLight: "Sáng",
      themePerformanceGreen: "Performance Green",
      themeElectricBlue: "Electric Blue",
      themeVoltLime: "Volt Lime",
      themeIronOrange: "Iron Orange",
      themeBlackVolt: "Black + Volt",
      themeCrimsonPerformance: "Crimson Performance",
      themeDark: "Tối",
      themeSystem: "Hệ thống",
      english: "EN",
      vietnamese: "VI",
      openNavigation: "Mở điều hướng",
      closeNavigation: "Đóng điều hướng",
      expandSidebar: "Mở rộng thanh bên",
      collapseSidebar: "Thu gọn thanh bên",
      settings: "Cài đặt",
      save: "Lưu",
      saving: "Đang lưu...",
      cancel: "Hủy",
      dismissNotification: "Đóng thông báo",
      edit: "Chỉnh sửa",
      delete: "Xóa",
      loading: "Đang tải...",
      tryAgain: "Thử lại",
      today: "Hôm nay",
      selectedDay: "Ngày đã chọn",
      profile: "Hồ sơ",
      signOut: "Đăng xuất",
      signingOut: "Đang đăng xuất...",
      addCoach: "Thêm huấn luyện viên",
      saveChanges: "Lưu thay đổi",
      sendResetEmail: "Gửi email đổi mật khẩu",
      sendingEmail: "Đang gửi email...",
      noData: "Chưa có dữ liệu",
    },
    shell: {
      dashboard: "Bảng điều khiển",
      weeklySchedule: "Lịch tuần",
      workout: "Buổi tập",
      mealTracking: "Theo dõi bữa ăn",
      progress: "Tiến độ",
      trainees: "Học viên",
      clients: "Khách hàng",
      programs: "Giáo án",
      exercises: "Bài tập",
      stats: "Thống kê",
      analytics: "Phân tích",
      adminDashboard: "Bảng quản trị",
      overview: "Tổng quan",
      users: "Người dùng",
      coachRequests: "Yêu cầu coach",
      coachSignups: "Đăng ký coach",
      connections: "Kết nối",
      audit: "Nhật ký",
      controlCenter: "Trung tâm quản lý",
      account: "Tài khoản",
      backToAthleteView: "Quay lại giao diện học viên",
      addClient: "Thêm học viên",
      activeClients: "học viên hoạt động",
      home: "Trang chủ",
      coach: "Coach",
      meals: "Bữa ăn",
      schedule: "Lịch",
      workouts: "Tập luyện",
      nutrition: "Dinh dưỡng",
      routine: "Routine",
      more: "Thêm",
      admin: "Quản trị",
      routines: "Bài tập",
      records: "Kết quả",
      body: "Cơ thể",
      startWorkout: "Bắt đầu",
      yeahBuddyUser: "Người dùng YeahBuddy",
      loadingEmail: "Đang tải...",
      loadingWorkspace: "Đang tải workspace",
    },
  },
} as const
