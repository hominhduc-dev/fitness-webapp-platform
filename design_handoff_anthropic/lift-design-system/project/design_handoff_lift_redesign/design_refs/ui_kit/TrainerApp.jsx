...
/* -------- Client detail ---------------------------------------------- */
function ClientDetail({ client, onBack, mobile }) {
  if (!client) {
    return (
    ...
    <div style={{ padding: mobile ? '16px 16px 32px' : '28px 36px', overflowY: 'auto' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 24, flexWrap: 'wrap' }}>
        ...
        {!mobile && (
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            <Button variant="dark" icon="plus">Assign workout</Button>
          </div>
        )}
      </div>
      ...
      {mobile && (
        <div style={{ display: 'flex', gap: 10, marginTop: 20 }}>
          ...
          <Button variant="dark" icon="plus" style={{ flex: 1, justifyContent: 'center' }}>Assign workout</Button>
        </div>
      )}
    </div>
  );
    ...
    <div style={{ display: 'flex', minHeight: '100vh', background: 'var(--ink-0)' }}>
      ...
      <main style={{ flex: 1, display: 'grid', gridTemplateColumns: '360px 1fr', minWidth: 0 }}>
        ...
        <div style={{ overflowY: 'auto', maxHeight: '100vh' }}>
          <ClientDetail client={current} mobile={false} />
        </div>
      </main>
    </div>
  );
