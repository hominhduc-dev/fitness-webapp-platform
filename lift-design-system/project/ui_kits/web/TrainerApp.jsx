...
function TrainerApp() {
  ...
  const renderStatsView = () => mobile
    ? <TrainerStats />
    : <main style={{ flex: 1, minWidth: 0, overflowY: 'auto', maxHeight: '100vh' }}><TrainerStats /></main>;

  const renderScheduleView = () => mobile
    ? <TrainerSchedule />
    : <main style={{ flex: 1, minWidth: 0, overflowY: 'auto', maxHeight: '100vh' }}><TrainerSchedule /></main>;

  const content =
    nav === 'clients'  ? renderClientsView() :
    nav === 'programs' ? renderProgramsView() :
    nav === 'sched'    ? renderScheduleView() :
    nav === 'stats'    ? renderStatsView() :
    renderClientsView();
