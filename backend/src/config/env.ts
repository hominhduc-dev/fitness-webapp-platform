...
const raw = {
  ...
  GOOGLE_OAUTH_CLIENT_ID: clean(process.env.GOOGLE_OAUTH_CLIENT_ID),
  ...
  NOTION_API_VERSION: clean(process.env.NOTION_API_VERSION),
  NOTION_PROGRAM_DB_ID: clean(process.env.NOTION_PROGRAM_DB_ID),
  NOTION_PROGRAM_ROWS_DB_ID: clean(process.env.NOTION_PROGRAM_ROWS_DB_ID),
  NOTION_TIMEOUT_MS: clean(process.env.NOTION_TIMEOUT_MS),
...
const envSchema = z.object({
  ...
  NOTION_PROGRAM_DB_ID: z.string().optional(),
  NOTION_PROGRAM_ROWS_DB_ID: z.string().optional(),
  NOTION_TIMEOUT_MS: numberFromString(15000).pipe(z.number().positive("NOTION_TIMEOUT_MS must be > 0")),
...
function loadEnv() {
  ...
  return {
    ...
    // Notion import is optional: when the token or the template database id is
    // missing the coach endpoints report the feature as unconfigured instead of
    // failing at boot, so a deployment without Notion still starts.
    notionApiVersion: parsed.NOTION_API_VERSION,
    notionProgramDbId: parsed.NOTION_PROGRAM_DB_ID,
    notionProgramRowsDbId: parsed.NOTION_PROGRAM_ROWS_DB_ID,
    notionTimeoutMs: parsed.NOTION_TIMEOUT_MS,
